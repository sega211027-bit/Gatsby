...
if not is_admin:
    if os.path.exists(JSON_FILE):
        ...
        with col1:
            ...
            # 토글: 이 값이 변경되면 rerun이 발생하며 아래 JS의 loopActive에 즉시 반영됨
        ...
        with col2:
            ...
            # [핵심] TypeError 방지를 위해 key를 삭제함.
            # 대신 스크립트 src에 reloader를 붙여 버튼 클릭 시 강제 리로드를 유도하고,
            # JS 변수들에 직접 파이썬 변수 값을 주입하여 구간 정보를 갱신함.
            yt_html = f"""
            <div id="player"></div>
            ...
            <script>
                ...
                function onPlayerStateChange(event) {{
                    // 재생 중(PLAYING) 상태일 때만 끝 지점 감시 시작
