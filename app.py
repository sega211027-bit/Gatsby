...
if os.path.exists(JSON_FILE):
    ...
    day_options = sorted(list(set(str(d['Day']) for d in r_data)))
    day = st.sidebar.select_slider("📅 Day", options=day_options)
    
    rnd_options = sorted(list(set(str(d['ROUND']) for d in r_data if str(d['Day']) == day)))
    rnd = st.sidebar.select_slider("🔁 Round", options=rnd_options)
