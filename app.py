...
# 다시 재생 버튼 클릭 시 숫자를 올려 브라우저가 콘텐츠 변화를 감지하게 함
...
if not is_admin:
    if os.path.exists(JSON_FILE):
        ...
        with col1:
            ...
            # 토글 변경 시에도 영상이 유지되도록 설계
            do_loop = st.checkbox("🔄 구간 무한 반복 재생", value=True)
            
            # 버튼 클릭 시 현재 분량의 시작점(start_sec)으로 리로드
        ...
        with col2:
            ...
            # [해결] f-string 에러 방지를 위해 {{ }} 사용 및 API 강제 로드 로직 추가
            yt_html = f"""
            <div id="player"></div>
            <script>
                // API 스크립트가 중복 로드되지 않도록 처리하며 강제 실행
            ...
            # 대신 reloader를 key처럼 활용하여 버튼 클릭 시에만 다시 그리게 함
