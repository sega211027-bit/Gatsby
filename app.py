...
# 재생 버튼 클릭 시 숫자를 올려서 브라우저가 새 콘텐츠로 인식하게 유도
...
if not is_admin:
    if os.path.exists(JSON_FILE):
        ...
        with col1:
            ...
            # 토글: 이 값은 자바스크립트 조건문으로 전달됨
            do_loop = st.checkbox("🔄 구간 무한 반복 재생", value=True)
            
            # [핵심] 버튼 클릭 시 reloader를 증가시켜 iframe을 강제 갱신
        ...
        with col2:
            ...
            # [중요] 중괄호를 {{ }}로 이스케이프하여 SyntaxError 해결
            # API 주소 뒤에 ?v={reloader}를 붙여 브라우저가 매번 새 스크립트로 인식하게 함
            yt_html = f"""
            <div id="player"></div>
            ...
            # TypeError의 주범인 'key'를 제거하고 st.empty() 공간에 렌더링
